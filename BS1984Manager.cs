...
namespace $safeprojectname$
{
    class BS1984Manager{
        public static void BS1984ManagerThread()
        {
            ...
            Console.Clear();
            Thread.Sleep(1000);
            Program.textRenderer("[Buddy Simulator 1984 Saves Manager] [Version 1.0]\nCreated By HackerRouter\nEnter HELP for more assistance.\n");
            while (true)
            {
                    ...
                    default:
                        if (commandInput.StartsWith("backup"))
                        {
                            ...
                            if (File.Exists(backupToDelete) == true)
                            {
                                while (true)
                                {
                                    Program.textRenderer($"Are you sure you would like to delete the backup [{backupToDelete}]?\n[WARNING: THIS ACTION CANNOT BE UNDONE]\n\n>");
                                    string responseInput = Console.ReadLine().ToLower();
                                    if (Program.yesResponses.Contains(responseInput))
                                    {
                            ...
                            {
                                Program.textRenderer($"[ERROR] Unable to find the backup[{backupToDelete}].\n");
                            }
                            break;
                        }
